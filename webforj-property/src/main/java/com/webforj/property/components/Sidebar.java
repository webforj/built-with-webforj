 ...
 */
public class Sidebar extends Composite<Div> {

  private final Div self = getBoundComponent();
  private final Div header;
  private final Span countLabel;
  private final Div listContainer;
  ...
  /**
   * Gets the property cards currently displayed.
   *
  ...
  /** Shows all properties (clears filter). */
  public void showAllProperties() {
    for (PropertyCard card : propertyCards) {
      card.setCardVisible(true);
    }
    updateCount(propertyCards.size());
    listContainer.setVisible(true);
