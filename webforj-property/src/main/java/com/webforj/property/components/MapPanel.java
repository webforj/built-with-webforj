...
public class MapPanel extends Composite<Div> {
  ...
  public MapPanel() {
    ...
    map = new Map();
    map.addClassName("map-panel__map");
    initializeMap();
    setupEventListeners();
  ...
  /**
   * Removes a property marker from the map.
   *
   * @param id the property ID to remove
   */
  public void removePropertyMarker(String id) {
    PropertyMarkerData data = propertyData.remove(id);
  ...
  /**
   * Gets the underlying Map component.
   *
  ...
  /** Clears all property markers from the map. */
  public void clearMarkers() {
    for (PropertyMarkerData data : propertyData.values()) {
      if (data.isVisible()) {
        map.removeFeature(data.getActiveMarker());
      }
    }
  ...
  /** Shows all property markers. */
  public void showAllMarkers() {
    setMarkerVisibility(new HashSet<>(propertyData.keySet()));
  }

  /**
   * Checks if a property marker is currently visible.
   *
   * @param propertyId the property ID to check
   * @return true if the marker is visible
   */
  public boolean isMarkerVisible(String propertyId) {
    PropertyMarkerData data = propertyData.get(propertyId);
    return data != null && data.isVisible();
  }
